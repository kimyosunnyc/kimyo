...
if(!function_exists('etheme_cart_items')) {
	function etheme_cart_items ($limit = 3) {
            ...
            foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
                ...
                ?>
                    <div class="product-item">       
						...
						<?php endif; ?>
                        <?php 
                            echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf('<a href="%s" data-key="%s" class="delete-btn" title="%s"><i class="icon-remove"></i></a>', esc_url( WC()->cart->get_remove_url( $cart_item_key ) ), $cart_item_key, __('Remove this item', ETHEME_DOMAIN) ), $cart_item_key ); 
                        ?>
        ...
        if ( sizeof( WC()->cart->get_cart() ) > 0 ) {
          ?>
            <div class="totals">
            	<span class="items left"><?php echo WC()->cart->cart_contents_count; ?> <?php _e('items', ETHEME_DOMAIN);?></span>
                <?php echo __('Total:', ETHEME_DOMAIN); ?> &nbsp;<span class="price"><span class="pricedisplay"><?php echo WC()->cart->get_cart_subtotal(); ?></span></span>
            </div>
          <?php
            ...
            ?>
                ...
                <a href="<?php echo WC()->cart->get_cart_url(); ?>" class="button left"><span><?php echo __('View Cart', ETHEME_DOMAIN); ?></span></a>  
                <a href="<?php echo WC()->cart->get_checkout_url(); ?>" class="button filled active right "><span><?php echo __('Checkout', ETHEME_DOMAIN); ?></span></a>   
